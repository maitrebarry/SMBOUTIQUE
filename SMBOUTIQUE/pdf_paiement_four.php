...
class PDF extends FPDF
{
    ...
    function header()
    {
        ...
        if (isset($_GET['detail'])) {
            ...
            $this->Cell(0, 15, '', 0, 1, 'C'); // Ligne vide pour créer de l'espace au-dessus du texte
             $this->Ln(15);
            $textWidth = $this->GetStringWidth('PAIEMENT N : ' . strtoupper($dpaie['paie_referrence']));
            $this->Rect(($this->w - 10 - $textWidth) / 2, $this->GetY(), $textWidth + 10, 10, 'D'); // Encadrement centré autour du texte
            $this->Cell(0, 10, 'PAIEMENT N : ' . strtoupper($dpaie['paie_referrence']), 0, 1, 'C'); // Centrer le texte
    ...
    function viewTable()
    {
        ...
        if (isset($_GET['detail'])) {
            ...
            $this->SetFont('Times', 'B', 9);
            $formatted_date = date('d/m/Y \a H:i', strtotime($dpaie['date_paie']));
            $this->Cell(0, 10, 'FOURNISSEUR: ' . $dpaie['nom_fournisseur'] . ' ' . $dpaie['prenom_fournisseur'] . '   PAYE le: ' . $formatted_date, 0, 1, 'C');
        }
    }
