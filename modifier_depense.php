    ...
    $recuperer = $update->recuperation_fonction("*", "depense WHERE id_depense=:id", [':id' => $_GET['id']]);

    if (isset($_POST['modifier'])) {
        extract($_POST);

        // Récupération de l'ancien montant de la dépense
        $ancienMontant = $recuperer->montant;

        // Modification
        $update->update_data("UPDATE depense
            SET 
                reference_caisse=:reference_caisse,
                libelle=:libelle,
                montant=:montant,
                date=:date,
                note=:note
        ...
        // Calcul de la différence entre l'ancien montant et le nouveau montant
        $differenceMontant = $montant - $ancienMontant;

        // Mise à jour du montant total de la caisse en fonction de l'augmentation ou de la diminution
        $update->update_data("UPDATE caisse
            SET 
                Montant_total_caisse = Montant_total_caisse - :differenceMontant
            WHERE reference_caisse = :reference_caisse",
            [
    ...
    echo '<script>
            document.addEventListener("DOMContentLoaded", function () {
                Swal.fire("Modification effectuée avec succès!", "", "success").then(() => {
                    window.location.href = "liste_depense_caisse.php";
                });
            });
          </script>';
    }
  ...
  <main id="main" class="main" >
       <div class="pagetitle">
            ...
            <nav>
                ...
                <li class="breadcrumb-item active">Modifier Dépenses</li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
   <div class="row">
            ...
            <div class="card-body">
                ...
                <form method="POST" >
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-3">
                            <input type="text" name="ref_caisse" class="form-control"value="<?=$recuperer->reference_caisse?>"> 
                        </div>
                        <label for="inputText" class="col-sm-3 col-form-label">Libellé(50 lettres au max)<span class="text-danger">*</span></label>
                        <div class="col-sm-4">
                            <input type="text" name="libelle" class="form-control" value="<?=$recuperer->libelle?>">
                        </div>
                    </div>
                    <div class="row mb-4">
                        <label for="inputNumber" class="col-sm-2 col-form-label">Montant<span class="text-danger">*</span></label>
                        <div class="col-sm-4">
                            <input type="number" name="montant" class="form-control"value="<?=$recuperer->montant?>">
                        </div>
                     ...
                     <div class="row mb-3">
                        ...
                        <div class="col-sm-8">
                            <textarea class="form-control" name="note" style="height: 100px"><?=$recuperer->note?></textarea>
                        </div>
                    </div>
                    <div class="text-center">
                        <button type="submit" class="btn btn-info"  name="modifier">Enregistrer</button>
